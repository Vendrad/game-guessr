...
describe('<GameInput />', () => {
  ...
  it('should pass autocompleteitemwasclickedhandler to GameAutoComplete.', () => {
    wrapper.setState({ inputHasFocus: true, inputValue: 'Over 3 Characters' });
    expect(
      wrapper.find(GameAutoComplete).getElement().props
        .autoCompleteItemWasClicked
    ).toBe(wrapper.instance().autoCompleteItemWasClickedHandler);
  });
  ...
  it('should fire callbacks if the input value handler was called with a new value.', () => {
    ...
    expect(defaultProps.inputWasChanged).toHaveBeenCalledWith(
      event.target.value
    );
  });

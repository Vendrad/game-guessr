...
describe('<GameMode />', () => {
  ...
  beforeEach(() => {
    ...
    defaultProps = {
      gameMode: {id: 0, type: 'normal', title: '80s', slug: '80', apiSlug: '0'},
  ...
  it('should render with the given title.', () => {
    expect(wrapper.find('p').getElement().props.children).toBe(defaultProps.gameMode.title);
  });

  it('should fire the callback when it is clicked.', () => {
    wrapper.find('.GameMode').simulate('click');
    expect(defaultProps.gameModeWasSelected).toHaveBeenCalledWith(defaultProps.gameMode.id);
  });

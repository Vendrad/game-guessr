...
describe('<GameAutoCompleteItem />', () => {
  ...
  it('should display the game name.', () => {
    expect(wrapper.find('p').getElement().props.children).toBe(
      defaultProps.gameName
    );
  });

  it('should display the cover if present.', () => {
    expect(wrapper.find('img').getElement().props.src).toBe(
      defaultProps.gameCover
    );
  });

  it('should display a backup image if the cover is not present.', () => {
    wrapper.setProps({ gameCover: undefined });
  ...
  it('should trigger callback on click.', () => {
    ...
    expect(defaultProps.gameAutoCompleteItemWasClicked).toHaveBeenCalledWith({
      id: defaultProps.gameId,
      name: defaultProps.gameName,
    });
  });
});
